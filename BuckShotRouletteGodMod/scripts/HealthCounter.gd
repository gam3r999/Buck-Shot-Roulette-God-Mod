...
func UpdateDisplayRoutineCigarette_Main(isChanging : bool, isAddingEnemy : bool):
	...
	if (!overriding_medicine):
		...
		# GOD MODE: Medicine only plays healing sound for player, never damage sound
		if (isChanging && overriding_medicine_adding): speaker_beep.play()
		#else: if(isChanging && !overriding_medicine_adding): speaker_noise.play()  # DISABLED
	await get_tree().create_timer(.7, false).timeout
	if (!isAddingEnemy): 
