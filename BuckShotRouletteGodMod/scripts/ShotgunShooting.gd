...
func Shoot(who : String):
	...
	if (currentRoundInChamber == "live" && who == "self"): 
		...
		# ALL DAMAGE AND DEATH CODE COMMENTED OUT - PLAYER IS INVINCIBLE
		...
		#playerDied = true
		#roundManager.health_player -= roundManager.currentShotgunDamage  # THIS LINE DOES DAMAGE - DISABLED
		#if (roundManager.health_player < 0): roundManager.health_player = 0
		playerCanGoAgain = false
		playerDied = false  # NEVER DIES
		#healthCounter.checkingPlayer = true
		#await(death.Kill("player", false, true))  # NO DEATH SEQUENCE
	# ========== GOD MODE END ==========
