...
func CheckIfOutOfHealth():
	...
	if (outOfHealth):
		#if (outOfHealth_player): OutOfHealth("player")  # DISABLED
...
func OutOfHealth(who : String):
	# ========== GOD MODE: PLAYER NEVER RUNS OUT OF HEALTH ==========
	if (who == "player"): 
		# Player can never die - skip death routine
		return  # DISABLED: death.MainDeathRoutine()
	else:
