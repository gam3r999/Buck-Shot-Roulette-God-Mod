...
func EndDealerTurn(canDealerGoAgain : bool):
	...
	# ========== GOD MODE: PLAYER CAN NEVER BE OUT OF HEALTH ==========
	var outOfHealth_player = false  # Always false - player can't die
	var outOfHealth_enemy = roundManager.health_opponent == 0
	var outOfHealth = outOfHealth_player or outOfHealth_enemy
	if (outOfHealth):
		#if (outOfHealth_player): roundManager.OutOfHealth("player")  # DISABLED
...
func Shoot(who : String):
	...
	if (currentRoundInChamber == "live" && who == "self"): 
		...
		return
	# ========== GOD MODE: DEALER CAN NEVER DAMAGE PLAYER ==========
	if (currentRoundInChamber == "live" && who == "player"): 
		# DO NOT DECREASE PLAYER HEALTH - PLAYER IS INVINCIBLE
		#roundManager.health_player -= roundManager.currentShotgunDamage  # DISABLED
		#if (roundManager.health_player < 0): roundManager.health_player = 0  # DISABLED
		# Still play visual effects
		cameraShaker.Shake()
		smoke.SpawnSmoke("barrel")
		# DO NOT CALL DEATH - PLAYER NEVER DIES
		#await(death.Kill("player", false, false))  # DISABLED
		playerDied = false  # NEVER DIES
