...
func Kill(who : String, trueDeath : bool, returningShotgun : bool):
	...
	match(who):
		"player":
			...
			# Skip all player death logic completely
			return
			# All code below is disabled in god mode
			"""
			if (trueDeath):
				pass
			else:
		...
		"dealer":
			if (trueDeath):
				...
				if(shotgunShooting.roundManager.health_opponent == 0):
					...
					return
				# GOD MODE: Player can never reach 0 health
				#if (shotgunShooting.roundManager.health_player == 0):
				#	shotgunShooting.roundManager.OutOfHealth("player")
				#	return
...
func MedicineDeath():
	# ========== GOD MODE: PLAYER NEVER DIES FROM MEDICINE ==========
	return
	# All code below is disabled in god mode
	"""
	viewblocker.visible = true
...
func MainDeathRoutine():
	# GOD MODE: Player should never reach this, but keep dealer logic
